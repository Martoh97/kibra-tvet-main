...
export const Departments = () => {
  ...
  const fetchDepartments = async () => {
    const { data } = await supabase
      .from("site_content")
      .select("*")
      .like("content_key", "dept_%")
      .eq("is_published", true);

    if (data && data.length > 0) {
      const updatedDepts = defaultDepartments.map((dept) => {
        const dbContent = data.find((d) => d.content_key === dept.key);
        if (dbContent) {
          return {
            ...dept,
            name: dbContent.title || dept.name,
            description: dbContent.description || dept.description,
            image: dbContent.image_url || dept.image,
            link: dbContent.link || dept.link,
          };
        }
        return dept;
      });
            ...
            >
              {isAdmin && (
                <InlineEditButton
                  ...
                  currentDescription={dept.description}
                  currentImageUrl={typeof dept.image === "string" ? dept.image : ""}
                  currentLink={dept.link}
